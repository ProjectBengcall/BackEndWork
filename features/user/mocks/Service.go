...
// UpdateProfile provides a mock function with given fields: updatedUser, file, fileheader, userID
func (_m *Service) UpdateProfile(updatedUser domain.UserCore, file multipart.File, fileheader *multipart.FileHeader, userID uint) (domain.UserCore, error) {
	ret := _m.Called(updatedUser, file, fileheader, userID)

	var r0 domain.UserCore
	if rf, ok := ret.Get(0).(func(domain.UserCore, multipart.File, *multipart.FileHeader, uint) domain.UserCore); ok {
		r0 = rf(updatedUser, file, fileheader, userID)
	} else {
		r0 = ret.Get(0).(domain.UserCore)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(domain.UserCore, multipart.File, *multipart.FileHeader, uint) error); ok {
		r1 = rf(updatedUser, file, fileheader, userID)
	} else {
		r1 = ret.Error(1)
	}
